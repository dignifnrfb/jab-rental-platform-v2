...
# 修改数据库连接配置
sed -i 's|DATABASE_URL="postgresql://username:password@localhost:5432/database_name"|DATABASE_URL="postgresql://jab_user:jab_secure_2024@localhost:5432/jab_rental"|g' .env
sed -i 's|NEXTAUTH_SECRET="your_nextauth_secret_key_here"|NEXTAUTH_SECRET="$(openssl rand -base64 32)"|g' .env
sed -i 's|NEXTAUTH_URL="http://localhost:3000"|NEXTAUTH_URL="http://$(curl -s ifconfig.me):3000"|g' .env
sed -i 's|NODE_ENV=production|NODE_ENV=production|g' .env

# 验证环境变量配置
echo "验证环境变量配置..."
if grep -q "DATABASE_URL=\"postgresql://jab_user:jab_secure_2024@localhost:5432/jab_rental\"" .env; then
    echo "✅ DATABASE_URL 配置正确"
else
    echo "❌ DATABASE_URL 配置错误，请检查"
    exit 1
fi
